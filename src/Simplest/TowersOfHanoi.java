package simplest;

import java.util.Stack;

public class TowersOfHanoi {
    private final int numDiscs;
    public final Stack<Integer> towerA = new Stack<Integer>();
    public final Stack<Integer> towerB = new Stack<Integer>();
    public final Stack<Integer> towerC = new Stack<Integer>();

    public TowersOfHanoi(int discs) {
        numDiscs = discs;
        for (int i = 0; i < discs; i++) {
            towerA.push(i);
        }
    }

    private void move(Stack<Integer> begin, Stack<Integer> end, Stack<Integer> temp, int n) {
        if (n == 1) {
            end.push(begin.pop());
        } else {
            move(begin, temp, end, n - 1);
            move(begin, end, temp, 1);
            move(temp, end, begin, n - 1);
        }
    }

    public void solve() {
        move(towerA, towerB, towerC, numDiscs);
    }

    public static void main(String... strings) {
        TowersOfHanoi hanoi = new TowersOfHanoi(3);
        hanoi.solve();
        System.out.println(hanoi.towerA);
        System.out.println(hanoi.towerB);
        System.out.println(hanoi.towerC);
    }
}
